...
import ubicomp.rehabdiary.utility.system.check.WeekNumCheck;
...
public class HttpPostGenerator {
	...
	public static HttpPost genPost(){
		...
		PackageInfo pinfo;
		try {
			pinfo = KetdiaryApplication.getContext().getPackageManager()
					.getPackageInfo(KetdiaryApplication.getContext().getPackageName(), 0);
...
//		builder.addTextBody("data[]", String.valueOf(0));
	...
	public static HttpPost genPost(IdentityScore data){
		...
		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		nvps.add(new BasicNameValuePair("uid", uid));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.getCreateTime().getTimeInMillis())));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.getScore())));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.getEventTime().getTimeInMillis())));
	...
	public static HttpPost genPost(EventLogStructure data){
		HttpPost httpPost = new HttpPost(ServerUrl.getEventLogUrl());
		String uid = PreferenceControl.getUID();
		String deviceId = PreferenceControl.getDeviceId();
		...
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.originalEmotion)));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.originalThought)));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.expectedBehavior)));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.expectedEmotion)));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data.expectedThought)));
