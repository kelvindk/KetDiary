...
public class SettingActivity extends Activity {
	...
	private RelativeLayout mainBase;
	...
	private RelativeLayout[] passwordViews;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mainBase = (RelativeLayout)this.findViewById( R.id.setting_base);
		...
		View title = BarButtonGenerator.createTitleView(getResources().getString(R.string.setting_title));
	...
	private void setting() {
		...
		/*String[] strs = KetdiaryApplication.getContext().getResources()
		...
		if(! ori_deviceid.equals("device_default"))
		{
			intID = Integer.valueOf(ori_deviceid.substring(ori_deviceid.length()-3));
		}
				...
				R.string.setting_password, new OnClickListener() {
					...
					public void onClick(View v) {
						...
						ImageView list = (ImageView) v
								.findViewById(R.id.question_list);
						if (visible) {
							for (int i = 0; i < passwordViews.length; ++i)
								passwordViews[i].setVisibility(View.GONE);
							list.setVisibility(View.INVISIBLE);
						} else {
							for (int i = 0; i < passwordViews.length; ++i)
								passwordViews[i].setVisibility(View.VISIBLE);
							list.setVisibility(View.VISIBLE);
						}
						visible = !visible;
					}
		...
		passwordViews = new RelativeLayout[2];

		passwordViews[0] = createPasswordView(0);
		passwordViews[0].setVisibility(View.GONE);

		mainLayout.addView(passwordViews[0]);

		passwordViews[1] = createPasswordView(1);
		passwordViews[1].setVisibility(View.GONE);
		mainLayout.addView(passwordViews[1]);

		passwordViews[0].setOnClickListener(new OnClickListener(){
		...
		passwordViews[1].setOnClickListener(new OnClickListener(){
	...
	private class RecreationOnClickListener implements View.OnClickListener {
		...
		private String ok = KetdiaryApplication.getContext().getString(R.string.ok);
		private String edit = KetdiaryApplication.getContext().getString(R.string.edit);

		private int ok_color = KetdiaryApplication.getContext().getResources()
				.getColor(R.color.red);
		private int edit_color = KetdiaryApplication.getContext().getResources()
	...
	/*private RelativeLayout createPasswordView(int x) {
		...
		TextView text = (TextView) layout.findViewById(R.id.password_description);
		text.setTypeface(wordTypeface);
		if(x == 0)
			text.setText("開啟密碼功能");
		if(x == 1)
