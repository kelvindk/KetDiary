...
import ubicomp.ketdiary.utility.system.check.NetworkCheck;
...
public class DataUploader {
	...
	public static void upload() {
		...
		if (ubicomp.ketdiary.utility.data.upload.SynchronizedLock.sharedLock.tryLock()) {
	...
	public static class DataUploadTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			...
			//EventLog
			EventLogStructure eventLogStructure[] = db.getNotUploadedEventLog();
			if (eventLogStructure != null) {
				for (int i = 0; i < eventLogStructure.length; ++i) {
					if (connectToServer(eventLogStructure[i]) == ERROR)
						Log.d(TAG, "FAIL TO UPLOAD - IdentityScore");
				}
			}
